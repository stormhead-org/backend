...
func (s *UserServer) ListCommunities(ctx context.Context, req *protopkg.ListUserCommunitiesRequest) (*protopkg.ListUserCommunitiesResponse, error) {
	if req.Limit <= 0 || req.Limit > 50 {
		req.Limit = 50
	}

	sortBy := ""
	if req.SortBy == protopkg.ListUserCommunitiesRequest_POPULARITY {
		sortBy = "popularity"
	}
	...
	if err != nil {
		s.log.Error("internal error listing user communities", zap.Error(err))
